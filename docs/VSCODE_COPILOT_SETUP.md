...
Ran `Chat: Install Plugin From Source` with `mathismeadows/roamer-mcp-plugin`. It now appears correctly under Extensions → Agent Plugins - Installed (enabled, with Disable/Uninstall/Open Plugin Folder/Open README actions). After that, the manual `"Roamer MCP"` entry was removed entirely from the user `mcp.json` and a fresh tool call (`whoami`) still succeeded — confirming the plugin's own bundled MCP server is genuinely auto-started and serving the connection, exactly as documented. **No manual `mcp.json` entry is present or needed.** The earlier "No agent plugins found" state was simply because the plugin had never actually been installed through this mechanism yet — installing it from source was the missing step, not a VS Code defect.
