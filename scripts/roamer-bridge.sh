...
# machines (AUTH-25) use a server-mediated device authorization flow, since
# Safari's HTTPS-Only Mode blocks a plain loopback redirect: RoamerMcp's
# server implements the RFC 8628 shape itself, showing a short code + URL
# (+ QR code) and polling until sign-in completes, with no local port or TLS
# certificate involved. Every other machine (AUTH-14) uses a lighter direct
